...
export default function Dashboard() {
  ...
  const { transaction, summary, isLoaderOn, chart } = useSelector(
  ...
  const updatedStats = summary?.summary
    ? stats.map((item) => {
        ...
        if (item.name === "Stock" && summary.summary?.equity) {
          ...
          changeType = summary.summary?.equity.change?.startsWith("+")
            ? "increase"
            : "decrease";
        }
        ...
        if (item.name === "Mutual fund" && summary.summary.mutual_fund) {
          ...
          changeType = summary.summary.mutual_fund.change?.startsWith("+")
            ? "increase"
            : "decrease";
        }
        ...
        if (item.name === "Bank" && summary.summary.bank) {
          ...
          changeType = summary.summary.bank.change
            ? summary.summary.bank.change.startsWith("+")
              ? "increase"
              : "decrease"
            : "neutral";
        }
      ...
      <dl className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {updatedStats.map((item) => (
          ...
          >
            ...
            <p className="mt-16 text-sm font-medium text-gray-600">
              {item.name}
            </p>
            ...
            >
              ...
              {item.changeType === "neutral" && (
                <span className="h-5 w-5 shrink-0 self-center text-gray-500">
                  •
                </span>
              )}
